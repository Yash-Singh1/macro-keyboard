/*
Adapted from the HID Keyboard demo code from BTstack
 */
...
static void queue_character(int character){
...
static void demo_text_timer_handler(btstack_timer_source_t * ts){
    ...
    for (int pin = 0; pin < 3; ++pin) {
        int new_keypress = gpio_get(pin);
        if (new_keypress != keypress[pin]) {
            keypress[pin] = new_keypress;
            if (!new_keypress) {
                if (pin == 0) queue_character(0x50);
                else if (pin == 1) queue_character(0x28);
                else if (pin == 2) queue_character(0x4F);
            }
        }
    }
    sleep_ms(10);
                ...
                case HCI_EVENT_HID_META:
                    switch (hci_event_hid_meta_get_subevent_code(packet)){
                        case HID_SUBEVENT_CONNECTION_OPENED:
                            ...
                            printf("HID Connected, waiting for keypresses...\n");
